		...
		while (read(input, &event, sizeof(event))==sizeof(event)) {
			...
			// TODO: tmp, hardcoded, missing some buttons
                    if (type==EV_KEY) {
                        ...
                        pressed = value;
                        if (code==305)      { btn = BTN_A;      id = BTN_ID_A; }
                        else if (code==304) { btn = BTN_B;      id = BTN_ID_B; }
                        else if (code==307) { btn = BTN_X;      id = BTN_ID_X; }
                        else if (code==308) { btn = BTN_Y;      id = BTN_ID_Y; }
                        else if (code==310) { btn = BTN_L1;     id = BTN_ID_L1; }
                        else if (code==311) { btn = BTN_R1;     id = BTN_ID_R1; }
                        else if (code==312) { btn = BTN_MENU;   id = BTN_ID_MENU; }
                        else if (code==313) { btn = BTN_R2;     id = BTN_ID_R2; }
                        else if (code==705) { btn = BTN_START;  id = BTN_ID_START; }
                        else if (code==704) { btn = BTN_SELECT; id = BTN_ID_SELECT; }
                        else if (code==544) { btn = BTN_DPAD_UP;    id = BTN_ID_DPAD_UP; }
                        else if (code==545) { btn = BTN_DPAD_DOWN;  id = BTN_ID_DPAD_DOWN; }
                        else if (code==546) { btn = BTN_DPAD_LEFT;  id = BTN_ID_DPAD_LEFT; }
                        else if (code==547) { btn = BTN_DPAD_RIGHT; id = BTN_ID_DPAD_RIGHT; }
                        else if (code==116) { btn = BTN_POWER;  id = BTN_ID_POWER; }
                        else { btn = BTN_NONE; }
                    }
			else if (type==EV_ABS) {
				...
				// TODO: these seem to be switched on the rgb30 according to the padtest rom
                            ...
                            if (code == 16) { // ABS_HAT0X
                                ...
                                else if (value == 1) { btn = BTN_DPAD_RIGHT; id = BTN_ID_DPAD_RIGHT; pressed = 1; }
                                else { 
                            ...
                            else if (code == 17) { // ABS_HAT0Y
                                if (value == -1) { btn = BTN_DPAD_UP; id = BTN_ID_DPAD_UP; pressed = 1; }
                                else if (value == 1) { btn = BTN_DPAD_DOWN; id = BTN_ID_DPAD_DOWN; pressed = 1; }
                                else { 
...
int PLAT_pickSampleRate(int requested, int max) {
	return MIN(MIN(requested, max), 44100);
}
