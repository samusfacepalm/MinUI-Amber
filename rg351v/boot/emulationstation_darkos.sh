                  ...
                  case $choice in
                          ...
                          "2)") if [ ! -z "$isitrgui" ]; then
                                  if [ ! -f "~/.config/.setxmb" ]; then
                                    ...
                                    if test ! -z "$(cat /home/ark/.config/.DEVICE | grep RGB20PRO | tr -d '\0')"
                                    then
                                      sed -i '/menu_scale_factor = /c\menu_scale_factor = \"1.60\"/g' /home/ark/.config/retroarch/retroarch.cfg
                                    else
                                      sed -i '/menu_scale_factor = /c\menu_scale_factor = \"1.30\"/g' /home/ark/.config/retroarch/retroarch.cfg
                                    fi
                                    sed -i '/xmb_menu_color_theme = \"4\"/s//xmb_menu_color_theme = \"8\"/g' /home/ark/.config/retroarch/retroarch.cfg
                                    touch /home/ark/.config/.setxmb
                                  fi
                                fi
