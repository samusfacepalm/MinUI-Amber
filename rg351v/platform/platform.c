...
#define RAW_X           306
#define RAW_Y           307
#define RAW_START       310
#define RAW_SELECT      311
#define RAW_MENU        313
#define RAW_L1          308
#define RAW_L2          314
#define RAW_L3          312
		...
		while (read(input, &event, sizeof(event))==sizeof(event)) {
			...
			// TODO: tmp, hardcoded, missing some buttons
                    if (type==EV_KEY) {
                        ...
                        pressed = value;
                        if (code==304)      { btn = BTN_A;      id = BTN_ID_A; }
                        else if (code==305) { btn = BTN_B;      id = BTN_ID_B; }
                        else if (code==306) { btn = BTN_X;      id = BTN_ID_X; }
                        else if (code==307) { btn = BTN_Y;      id = BTN_ID_Y; }
                        else if (code==308) { btn = BTN_L1;     id = BTN_ID_L1; }
                        else if (code==309) { btn = BTN_R1;     id = BTN_ID_R1; }
                        else if (code==314) { btn = BTN_L2;     id = BTN_ID_L2; }
                        else if (code==315) { btn = BTN_R2;     id = BTN_ID_R2; }
                        else if (code==310) { btn = BTN_START;  id = BTN_ID_START; }
                        else if (code==311) { btn = BTN_SELECT; id = BTN_ID_SELECT; }
                        else if (code==313) { btn = BTN_MENU;   id = BTN_ID_MENU; }
                        else if (code==116) { btn = BTN_POWER;  id = BTN_ID_POWER; }
                        else if (code==115) { btn = BTN_PLUS;   id = BTN_ID_PLUS; }
                        else if (code==114) { btn = BTN_MINUS;  id = BTN_ID_MINUS; }
                        else { btn = BTN_NONE; }
                    }
			else if (type==EV_ABS) {
